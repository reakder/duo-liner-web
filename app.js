...
function deleteCliente(id){if(!confirm("¿Eliminar cliente? También se eliminarán sus pedidos y pagos."))return;const db=getDB();db.clientes=db.clientes.filter(c=>c.id!==id);db.pedidos=db.pedidos.filter(p=>p.cliente_id!==id);db.pagos=db.pagos.filter(p=>p.cliente_id!==id);saveDB(db);renderAll()}
async function addPedido(){
  const cliente_id = document.getElementById("pedidoCliente").value;

  if(!cliente_id){
    return alert("Primero crea un cliente.");
  }

  const producto = document.getElementById("pedidoProducto").value.trim();

  if(!producto){
    return alert("Ingresa producto o descripción.");
  }
