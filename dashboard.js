...
async function login() {
  ...
  // Primero intenta login seguro contra backend /auth/login.
  try {
    ...
    console.warn("Login seguro no disponible todavía:", error);
  }

  // Acceso temporal antiguo mientras terminas de subir el parche del backend.
  // Cuando /auth/login esté activo en Render, puedes quitar este bloque.
  if (user === "admin" && pass === "1234") {
    localStorage.setItem("duoliner_session", "ok");
