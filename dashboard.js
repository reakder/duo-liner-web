...
function obtenerPedidosFiltrados(){
  ...
  return db.pedidos.filter(p=>{
    ...
    return (!filtroCliente||nombreCliente.includes(filtroCliente))&&(!filtroInicio||fechaPedido>=filtroInicio)&&(!filtroFin||fechaPedido<=filtroFin)&&(!filtroEstado||estadoPedido===filtroEstado);
  });
}
function limpiarFiltrosPedidos(){
  const cliente=document.getElementById("filtroClientePedido");
  const inicio=document.getElementById("filtroFechaInicio");
  const fin=document.getElementById("filtroFechaFin");
  const estado=document.getElementById("filtroEstadoPedido");
...
checkAuth();document.addEventListener("DOMContentLoaded",()=>{["filtroClientePedido","filtroFechaInicio","filtroFechaFin","filtroEstadoPedido"].forEach(id=>{const el=document.getElementById(id);if(el){el.addEventListener("input",renderAll);el.addEventListener("change",renderAll)}});if(location.pathname.includes("dashboard"))cargarMongoDB();else renderAll()});
